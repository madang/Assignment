...
[ct.gridX,ct.gridY,ct.gridZ]=ndgrid(1:size(ct.volume,2),1:size(ct.volume,1),1:size(ct.volume,3));% because sampling is isotropic with step 1(mm)
...
Xray=f_transform_my_grid(Xray,Xray.TPos);
ct=f_transform_my_grid(ct,ct.TPos);
...
plot3(Xray.gridX(1:ts:end),Xray.gridY(1:ts:end),Xray.gridZ(1:ts:end),'.','Color','b');
grid on;
hold on;
% Green for ct

plot3(ct.gridX(1:ts:end),ct.gridY(1:ts:end),ct.gridZ(1:ts:end),'.','Color','g');
